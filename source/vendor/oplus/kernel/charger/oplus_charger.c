...
static inline bool oplus_midas_charger_in_effect(void)
{
...
static void oplus_midas_chg_processing(void)
{
	static int inited = 0;
	int val, ret = 0;
	...
	if (jiffies_to_msecs(jiffies - prevt) >
			(OPLUS_CHG_UPDATE_INTERVAL_SEC -1) * 1000) {
		if (oplus_midas_charger_in_effect()) {
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	chip->disable_normal_charging_cool_down = of_property_read_bool(node, "qcom,disable_normal_charging_cool_down");
		...
		if (chip->fastchg_ffc_status == 1) {
			...
			if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL) {
				if ((chip->icharging * -1) < (chip->limits.ff1_normal_fastchg_ma
					...
					if (chip->chg_ops->get_charger_suspend && chip->chg_ops->get_charger_suspend() == true) {
						oplus_chg_unsuspend_charger();
					}

					if (chip->chg_ops->get_charging_enable() == false) {
						chip->chg_ops->charging_enable();
					}
...
int oplus_convert_current_to_level(struct oplus_chg_chip *chip, int val)
{
	...
	if(reply_bits == 7) {
		if(chip->vbatt_num == 2 || oplus_chg_get_gauge_and_asic_status() == 1) {
