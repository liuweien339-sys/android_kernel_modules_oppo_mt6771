...
static int zy0603_start_checksum_cal(void)
{
...
static int zy0603_disable_ssdf_and_pf(void)
{
	u8 write_ssdftime[5] = {0x29,0x41,0x00,0x95,0x05};
	u8 write_staticDFcheksum[6] = {0x7A,0x42,0x0F,0x06,0x2E,0x06};
	u8 pf_en_cmd[2] = {0x24,0x00};
	u8 mfgStatus_cm[2]={0x57,0x00};
	u8 read_buf[4] = {0};
	int rc = 0;
	int retry_cnt = 0;
	...
	if (!zy0603_unseal()) {	//ok
		...
		bq27541_write_i2c_block(0x3e, 4, write_staticDFcheksum);
		pr_err("%s write {0x7A,0x42,0x0F,0x06} --> 0x3e\n", __func__);
		usleep_range(5000, 5000);

		bq27541_write_i2c_block(0x60, 2, write_staticDFcheksum+4);
		pr_err("%s write {0x2E,0x06} --> 0x60\n", __func__);
		usleep_range(5000, 5000);

		bq27541_read_i2c_block(0x3E, 4, read_buf);
		pr_err("%s 0x3E -->read [0x%02x][0x%02x] [0x%02x][0x%02x]\n", __func__,
			read_buf[0], read_buf[1], read_buf[2], read_buf[2]);
		if(!((read_buf[0]==write_staticDFcheksum[0])&&(read_buf[1]==write_staticDFcheksum[1])&&(read_buf[2]==write_staticDFcheksum[2])&&(read_buf[3]==write_staticDFcheksum[3])))
		{
			rc = -1;
			return rc;
		}
		...
		do {
			...
			bq27541_write_i2c_block(0x3E, 2, mfgStatus_cm);
		...
		if(!((read_buf[0]==mfgStatus_cm[0])&&(read_buf[1]==mfgStatus_cm[1])&&((read_buf[3]&0x40)==0)))
		{
			rc = -1;
...
ssdf_check_done:
	...
	do {
		...
		error_flag = 0;
		if ((read_buf[0] == mfgStatus_cm[0] && read_buf[1] == mfgStatus_cm[1] && !(read_buf[2] & (0x1 << 6)))) {
...
#define ZY0603_QMAX_MIN	1700
#define ZY0603_QMAX_MAX	2443
static int zy0603_QmaxFgStatus_check(void)
{
	int rc = 0;
	...
	//check
	pr_err("[0x%0x] [0x%0x] [0x%0x] [0x%0x] [0x%0x] [0x%0x] [0x%0x]\n",
	...
	if (!(QmaxCell1 >= ZY0603_QMAX_MIN && QmaxCell1 <= ZY0603_QMAX_MAX) ||
		!(QmaxCell2 >= ZY0603_QMAX_MIN && QmaxCell2 <= ZY0603_QMAX_MAX)	||
		!(FGStatus == 0x33))
	{
		rc = 1;	//exception
...
static int zy0603_update_afi_buf(u16 base_addr, u16 len, u8 *afi_buf1, char *msg)
{
	u8 checksum = 0xFF;
	u8 len1 = len / 16;
	u8 len2 = len % 16;
	...
	if (len2 > 0) {											//tail: len2 < 16
		...
		do {
			...
			for (k=0; k<len2 + 2; k++)
			{
				pr_err("%s 2 [%d] = 0x%0x ", msg, k, read_buf[k]);
				error_flag = 0;
				if(write_buf[k] != read_buf[k]) {
					error_flag = 1;
					retry++;
...
static int zy0603_afi_param_update(void)
{
	u16 base_addr;

	//update....
	u8 afi_buf1[202] = {
	};

	u8 afi_buf2[43] = {
	0x64,0x63,0x64,0x61,0x00,0x01,0x01,0x02,0x05,0x21,0x1C,0x00,0xA2,0x40,0x05,0x57,0x20,0x00,0x00,0x03,0x00,0x00,0x0A,0xF0,0x0A,0x28,0x14,0x0A,0x14,0x10,0xE0,0x64,
	0x07,0xD0,0x00,0x03,0x0F,0x00,0x14,0x04,0x19,0xC8,0x00};

	u8 afi_buf3[67] = {
	0xAA,0x01,0xC8,0x03,0x8E,0x05,0xEF,0x07,0xE0,0x09,0x63,0x0B,0x44,0x0D,0xE7,0x0F,0x98,0x11,0xDA,0x13,0x7C,0x15,0x71,0x17,0x53,0x19,0x35,0x1B,0x52,0x3D,0xB6,0xB5,
	0x20,0xA9,0x22,0x2B,0x24,0x0C,0x26,0xC0,0x28,0x42,0x2A,0xA1,0x2C,0xE1,0x2E,0x64,0x30,0xFB,0x32,0x18,0x34,0x59,0x36,0x59,0x38,0x0A,0x3A,0x0A,0x1C,0xE8,0x07,0x93,
	0x07,0x93,0x33};

	u8 afi_buf4[20] = {0x11,0x38,0x11,0x38,0x00,0xFA,0xFD,0x7C,0xFE,0x20,0xFE,0xD5,0xFC,0x18,0x00,0x6A,0x00,0x32,0x00,0x32};

	u8 afi_buf5[2] = {0x0F,0x06};

	u8 afi_buf6[85] = {
	0xFF,0x01,0xC8,0x03,0x8E,0x05,0xEF,0x07,0xE0,0x09,0x63,0x0B,0x44,0x0D,0xE7,0x0F,0x98,0x11,0xDA,0x13,0x7C,0x15,0x71,0x17,0x53,0x19,0x35,0x1B,0x90,0x3D,0x9D,0xE0,
	0x20,0xA9,0x22,0x2B,0x24,0x0C,0x26,0xC0,0x28,0x42,0x2A,0xA1,0x2C,0xE1,0x2E,0x64,0x30,0xFB,0x32,0x18,0x34,0x59,0x36,0x59,0x38,0x0A,0x3A,0xB5,0x1C,0x50,0x01,0x01,
	0x00,0x00,0x00,0x00,0x00,0x33,0x00,0x27,0x00,0x46,0x00,0x3C,0x00,0x3C,0x01,0x3C,0xFF,0xF6,0xFE,0x0C,0x32};

	u8 afi_buf7[448] = {
	};

	//write afi_buf1
	base_addr = 0x4000;
	zy0603_update_afi_buf(base_addr, 202, afi_buf1, "afi_buf1");

	//write afi_buf2
	base_addr = 0x40FF;
	zy0603_update_afi_buf(base_addr, 43, afi_buf2, "afi_buf2");

	//write afi_buf3
	base_addr = 0x4200;
	zy0603_update_afi_buf(base_addr, 67, afi_buf3, "afi_buf3");

	//write afi_buf4
	base_addr = 0x4245;
	zy0603_update_afi_buf(base_addr, 20, afi_buf4, "afi_buf4");

	//write afi_buf5
	base_addr = 0x427A;
	zy0603_update_afi_buf(base_addr, 2, afi_buf5, "afi_buf5");

	//write afi_buf6
	base_addr = 0x4400;
	zy0603_update_afi_buf(base_addr, 85, afi_buf6, "afi_buf6");

	//write afi_buf7
	base_addr = 0x4600;
	zy0603_update_afi_buf(base_addr, 448, afi_buf7, "afi_buf7");
	return 0;
}
...
static bool zy0603_afi_update_done(void)
{
	...
	if (gauge_ic->batt_zy0603 && !gauge_ic->afi_update_done) {
...
int bq27541_get_passedchg(int *val)
{
	...
	bq27541_read_i2c_onebyte(0x4E, &value);
	v |= value;
	bq27541_read_i2c_onebyte(0x4F, &value);
	v |= (value << 8);
		...
		if (!chip->protect_check_done) {
			do {
				...
				if (GUAGE_OK != zy0603_disable_ssdf_and_pf_check()) {
						retry_cnt++;
						error_flag = 1;
						zy0603_disable_ssdf_and_pf();
				}
			} while (retry_cnt < 5 && error_flag == 1);
			if (gauge_ic && retry_cnt < 5) {
				gauge_ic->disabled = true;
				pr_err("lkl %s v12 zy0603_disable_ssdf_and_pf_check ok\n", __func__);
			} else {
				pr_err("lkl %s v12 zy0603_disable_ssdf_and_pf_check failed\n", __func__);
			}
		}

		if (!zy0603_seal()) {
			zy0603_start_checksum_cal();
		}
	}
