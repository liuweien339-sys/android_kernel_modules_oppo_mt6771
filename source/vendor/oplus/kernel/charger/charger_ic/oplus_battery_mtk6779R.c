...
static bool oplus_mt6360_get_vbus_status(void)
{
	...
	if (vbus_status == false && oplus_vooc_get_fastchg_started() == true) {
			chg_err("USBIN_PLUGIN_RT_STS_BIT low but fastchg started true\n");
			vbus_status = true;
	}
#endif
...
static void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	if (!pinfo->usb_temp_v_l_chan
		...
		chip->usbtemp_volt_r = USB_25C_VOLT;
		chip->usbtemp_volt_l = USB_25C_VOLT;
		return;
	}
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop() && dischg_flag == false) {
		...
		if ((chip->usb_temp_r >= USB_57C && chip->usb_temp_r < USB_100C)
			...
			count_l = 0;
			count = 0;
		} else if ((((chip->usb_temp_r * 10  - chip->temperature) > chip->limits.usb_high_than_bat_decidegc) && chip->usb_temp_r >= USB_20C && chip->usb_temp_r < USB_100C)
				|| (((chip->usb_temp_l * 10 - chip->temperature) >  chip->limits.usb_high_than_bat_decidegc) && chip->usb_temp_l >= USB_20C && chip->usb_temp_l < USB_100C)) {
			if (count <= time_count) {
				if (count == 0) {
