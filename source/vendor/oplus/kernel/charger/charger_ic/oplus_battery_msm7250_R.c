...
static int smbchg_get_usb_suspend(void)
{
	int rc = 0;
	u8 temp = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;

	rc = smblib_read(&chip->pmic_spmi.smb5_chip->chg, USBIN_CMD_IL_REG, &temp);
	if (rc < 0) {
		chg_err("Couldn't read USBIN_CMD_IL_REG rc=%d\n", rc);
		return 0;
	}

	rc = !!(temp & USBIN_SUSPEND_BIT);
...
struct oplus_chg_operations  smb5_chg_ops = {
	...
	.get_charger_suspend = smbchg_get_usb_suspend,
