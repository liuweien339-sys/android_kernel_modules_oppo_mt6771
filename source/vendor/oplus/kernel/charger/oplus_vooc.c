...
*/
	...
	if (data == VOOC_NOTIFY_FAST_PRESENT) {
		...
		chip->fast_chg_type_pre = chip->fast_chg_type;
	...
	ret_info = 0x2;
		...
		if (!chip->skip_high_soc_cool_down
				&& (chip->vooc_multistep_adjust_current_support == true) && (soc > 85)) {
...
void oplus_vooc_init(struct oplus_vooc_chip *chip)
{
	...
	chip->fast_chg_type = FASTCHG_CHARGER_TYPE_UNKOWN;
	chip->fast_chg_type_pre = FASTCHG_CHARGER_TYPE_UNKOWN;
	/*chip->batt_psy = power_supply_get_by_name("battery");*/
	chip->disable_adapter_output = false;
...
void oplus_vooc_reset_mcu(void)
{
	...
	if (!g_vooc_chip) {
		...
		if ((mcu_hwid_type == OPLUS_VOOC_ASIC_HWID_RK826
			...
			&& g_vooc_chip->vooc_reply_mcu_bits == 7) {
			usleep_range(100000, 100000);
			if (g_vooc_chip->vops->update_temperature_soc != NULL)
				g_vooc_chip->vops->update_temperature_soc();
		}
	}
}
...
int oplus_vooc_get_fast_chg_type_pre(void)
{
	struct oplus_vooc_chip *chip = g_vooc_chip;
	if (!chip) {
		return FASTCHG_CHARGER_TYPE_UNKOWN;
	} else {
		return chip->fast_chg_type_pre;
	}
}
