...
static int elevator_detect_data_report(struct data_unit_t *input_event)
{
    ...
    event.word[0] = input_event->oppo_data_t.elevator_detect_event.value;
    event.word[1] = input_event->oppo_data_t.elevator_detect_event.report_count;
...
static int elevator_detect_recv_data(struct data_unit_t *event, void *reserved)
{
    ...
    ELEVATOR_DETECT_LOG("elevator_detect recv data, flush_action = %d, value = %d, report_count = %d, timestamp = %llu\n",
        event->flush_action,
        event->oppo_data_t.elevator_detect_event.value,
        event->oppo_data_t.elevator_detect_event.report_count,
