...
static int lux_aod_data_report(struct data_unit_t *input_event)
{
    ...
    event.word[0] = input_event->oppo_data_t.lux_aod_event.state;
    event.word[1] = input_event->oppo_data_t.lux_aod_event.report_count;
...
static int lux_aod_recv_data(struct data_unit_t *event, void *reserved)
{
    ...
    LUX_AOD_KERNEL_LOG("recv data, flush_action = %d, state = %d, report_count = %d, timestamp = %lld\n",
        event->flush_action, event->oppo_data_t.lux_aod_event.state,
        event->oppo_data_t.lux_aod_event.report_count, (int64_t)event->time_stamp);
