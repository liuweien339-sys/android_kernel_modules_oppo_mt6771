...
static int fp_display_data_report(struct data_unit_t *input_event)
{
    ...
    event.word[0] = input_event->oppo_data_t.fp_display_data_t.value;
    event.word[1] = input_event->oppo_data_t.fp_display_data_t.report_count;
...
static int fp_display_recv_data(struct data_unit_t *event, void *reserved)
{
    ...
    FP_DISPLAY_LOG("recv data, flush_action = %d, value = %d, report_count = %d, timestamp = %lld\n",
        event->flush_action, event->oppo_data_t.fp_display_data_t.value,
        event->oppo_data_t.fp_display_data_t.report_count,
