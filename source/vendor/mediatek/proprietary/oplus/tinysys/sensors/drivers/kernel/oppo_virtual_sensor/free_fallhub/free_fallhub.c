...
** VENDOR_EDIT
** File: - free_fallhub.c
** Description: Source file for free_fall sensor linux driver.
** Version: 1.0
** Date : 2020/03/31
**
** --------------------------- Revision History: ---------------------
...
static int free_fall_data_report(struct data_unit_t *input_event)
{
    ...
    event.word[0] = input_event->oppo_data_t.free_fall_data_t.free_fall_time;
    event.word[1] = input_event->oppo_data_t.free_fall_data_t.angle;
    event.word[2] = input_event->oppo_data_t.free_fall_data_t.report_count;
...
static int free_fall_recv_data(struct data_unit_t *event, void *reserved)
{
    ...
    FREE_FALL_LOG("free_fall recv data, flush_action = %d, free_fall_time = %d, angle = %d, report_count = %d, timestamp = %lld\n",
        event->flush_action, event->oppo_data_t.free_fall_data_t.free_fall_time,
        event->oppo_data_t.free_fall_data_t.angle,
        event->oppo_data_t.free_fall_data_t.report_count, (int64_t)event->time_stamp);
